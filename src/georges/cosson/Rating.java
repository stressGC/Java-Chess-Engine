...
public class Rating {
	...
	// pieces values taken from https://github.com/Garee/jchess
	 ...
	 * => simply counts the pieces remaining on both sides, and computes the difference between them multiplied by the value per piece
	 */
    ...
    static int getEvaluation(Long W, Long B, int valuePerCapita) {
    	...
    	// count the number of piece in the bitboards
    	int scoreW = Long.bitCount(W);
    	int scoreB = Long.bitCount(B);
    	
    	// compare them, and multiply by the value per piece
